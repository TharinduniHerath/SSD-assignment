...
document.getElementById('file-input').value = '';
 ...
 const handleSubmit = async (e) => {
  ...
  if (!csrfToken) {
    toast.error("CSRF token not yet loaded. Try again.");
    return;
  }

  try {
    const URL = file ? await uploadImage(file) : imageURL;

    await userRequest.put(
      "/pets/" + id,
      { petID, petName, age, gender, species, breed, nic, customerName, contactNumber, medicalHistory, picture: URL },

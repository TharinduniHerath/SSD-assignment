...
const getPayroll = asyncHandler(async (req, res) => {
    ...
    if (qSearch) {
        payrolls = await Payroll.find({ $text: { $search: qSearch } });
    } else {
...
// @desc    Fetch a single payroll by ID
...
const updatePayroll = asyncHandler(async (req, res) => {
    ...
    ['staffId', 'otHours', 'salary', 'paymentStatus', 'date'].forEach((field) => {
        if (req.body[field] !== undefined) {
            allowedUpdates[field] = req.body[field];
        }
    });

    const updatedPayroll = await Payroll.findByIdAndUpdate(id, { $set: allowedUpdates }, { new: true });
